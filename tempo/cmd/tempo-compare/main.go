// tempo-compare compares a fixed transaction suite on two disposable localnets.
package main
...
// Storage fixture: store 42 in slot zero and return that slot.
const storageRuntime = "0x602a60005560005460005260206000f3"
const storageInit = "0x6010600c60003960106000f3602a60005560005460005260206000f3"
...
func run(ctx context.Context, endpoints [2]string, seed int64, out *json.Encoder) error {
	...
	names = append(names, "deploy-storage", "write-storage")
	for _, name := range names {
		...
		case "deploy-storage":
			deployed = crypto.CreateAddress(s.Address(), nonce)
			tx.Calls = []transaction.Call{{Value: new(big.Int), Data: common.FromHex(storageInit)}}
			expected.SetInt64(0)
		case "write-storage":
			tx.Calls = []transaction.Call{{To: &deployed, Value: new(big.Int)}}
			expected.SetInt64(0)
		}
		if err != nil {
			return err
		...
		for i, c := range clients {
			...
			if name == "deploy-storage" || name == "write-storage" {
				var code, slot hexutil.Bytes
				if err := c.CallContext(ctx, &code, "eth_getCode", deployed, "latest"); err != nil {
					return err
				}
				if hexutil.Encode(code) != storageRuntime {
					return fmt.Errorf("%s chain %d: unexpected deployed code", name, i)
				}
				if err := c.CallContext(ctx, &slot, "eth_getStorageAt", deployed, "0x0", "latest"); err != nil {
					return err
				}
				want := int64(0)
				if name == "write-storage" {
					want = 42
				}
				if new(big.Int).SetBytes(slot).Cmp(big.NewInt(want)) != 0 {
					return fmt.Errorf("%s chain %d: unexpected storage", name, i)
				}
				results[i].ContractCode = hexutil.Encode(code)
				results[i].Storage = hexutil.Encode(slot)
			}
		}
		if !reflect.DeepEqual(results[0], results[1]) {
			_ = out.Encode(map[string]any{"mismatch": name, "baseline": results[0], "candidate": results[1]})
...
func main() {
	baselineRPC := flag.String("baseline-rpc", "", "fresh baseline localnet RPC (loopback only)")
	candidateRPC := flag.String("candidate-rpc", "", "fresh candidate localnet RPC (loopback only)")
	seed := flag.Int64("seed", 1, "public deterministic fixture seed")
	flag.Parse()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	if err := run(ctx, [2]string{*baselineRPC, *candidateRPC}, *seed, json.NewEncoder(os.Stdout)); err != nil {
